...
object DemoRDD extends App {
  ...
  /**
   * Load data into RDD from parquet actual trip data file (src/main/resources/data/yellow_taxi_jan_25_2018).
   ...
   * Result: Data with the resulting table should appear in the console, a file should appear in the file system. Check out the solution in github gist.
   * */
  ...
  val filteredTaxiZoneRDD: RDD[TaxiZone] = taxiZoneRDD
    .filter(r => r != header)
    .map(l => l.split(","))
    .map(t => new TaxiZone(t(0).toInt, t(1), t(2), t(3)))
